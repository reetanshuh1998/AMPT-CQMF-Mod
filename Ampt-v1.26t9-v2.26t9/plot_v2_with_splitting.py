...
species = [
    ...
    ([PID['pr']],              'Protons',     'Table107.csv', r'STAR $p$ BES-I'),
    ([PID['pbar']],            'Antiprotons', None,           None),
]
...
# Plot first 3 species (Pions, Kaons, Protons)
for i, (_, label, star_file, star_label) in enumerate(species[:3]):
    ax = axs[i]
    # STAR BES-I data
    s_pt, s_v2, s_err = load_star_v2(star_file)
    if s_pt is not None:
        ax.errorbar(s_pt, s_v2, yerr=s_err, fmt='k*', mfc='black', mec='black', ecolor='black',
                    markersize=12, label=star_label, zorder=10)

    # Linear
    v2m_lin, v2e_lin = res_lin[label]
    ax.errorbar(pt_centers, v2m_lin, yerr=v2e_lin, color='#2196F3', marker='s', ls='none',
                markersize=6, capsize=2, elinewidth=0.8, label=r'Linear Extrapolation (Block)', zorder=8)

    # Gaussian
    v2m_gau, v2e_gau = res_gau[label]
    ax.errorbar(pt_centers, v2m_gau, yerr=v2e_gau, color='#E91E63', marker='D', ls='none',
                markersize=6, capsize=2, elinewidth=0.8, label=r'Gaussian Kernel (h=1.0)', zorder=9)

    ax.axhline(0, color='gray', ls='--', lw=0.8)
    ax.set_xlabel(r'$p_T$ (GeV/$c$)')
    ax.set_ylabel(r'$v_2 = \langle\cos 2\phi\rangle$')
    ax.set_title(f'{label} $v_2(p_T)$ (10-40%)')
    ax.set_xlim(0.0, 2.4)
...
if s_pt_sp is not None:
    ax.errorbar(s_pt_sp, s_v2_sp, yerr=s_err_sp, fmt='k*', label='STAR Splitting (10-40%)', markersize=12, zorder=10)
...
ax.errorbar(pt_centers, dv2_lin, yerr=edv2_lin, color='#2196F3', marker='s', ls='none',
            markersize=6, capsize=2, elinewidth=0.8, label=r'Linear Extrapolation (Block)', zorder=8)
...
ax.errorbar(pt_centers, dv2_gau, yerr=edv2_gau, color='#E91E63', marker='D', ls='none',
            markersize=6, capsize=2, elinewidth=0.8, label=r'Gaussian Kernel (h=1.0)', zorder=9)

ax.axhline(0, color='gray', ls='--', lw=0.8)
ax.set_xlabel(r'$p_T$ (GeV/$c$)')
ax.set_ylabel(r'$\Delta v_2 = v_2(p) - v_2(\bar{p})$')
ax.set_title(r'Proton-Antiproton Splitting $\Delta v_2$')
ax.set_xlim(0.0, 2.4)
ax.set_ylim(-0.15, 0.15)
ax.legend(fontsize=9, loc='upper left')

fig.suptitle(r'Smoothing Kernel Comparison: $v_2(p_T)$ & $\Delta v_2$ — $10\times10\times10$ grid vs STAR @ 7.7 GeV',
             fontsize=15, fontweight='bold', y=0.98)
fig.tight_layout()
outpath = f'{OUTDIR}/exact_pt_detailed_v2_all_models.png'
