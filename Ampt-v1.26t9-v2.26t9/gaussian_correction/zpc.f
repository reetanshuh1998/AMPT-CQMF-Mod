...
c3      update iopern, t
        ...
        iopern = iopern + 1
c --- CQMF LOCAL DENSITY: Phase 2 forces disabled until collision ---
...
c        if(izpc.eq.100) that=-4d0*pp2*ran1(2)
        ...
        if (iqmc .ge. 1 .and. pp2_use .gt. 0.0d0) then
...
c     Zero-initialize vector potentials (safe fallback for iqmc=0)
 ...
 882  continue
      ...
      if (iqmc .ge. 1) then
         open(89, file='model_data.csv', status='old', err=900)
         read(89, '(A)') line
 ...
 300     continue
      ...
      return
 900  iqmc = 0
      write(6,*) 'WARNING: File missing, using default ZPC.'
      return
      end
...
c     Map parton position to dynamic grid cell
      ...
      if(abs(ifl).eq.3) then
         ...
         v_out  = vs_c(ix,iy,iz)
      elseif(abs(ifl).eq.1) then
         xm_out = md_c(ix,iy,iz)
         v_out  = vd_c(ix,iy,iz)
      else
         xm_out = mu_c(ix,iy,iz)
         v_out  = vu_c(ix,iy,iz)
      endif
